...
func (w *Wallet) MultiMintPayment(request string, split map[string]uint64) ([]nut05.PostMeltQuoteBolt11Response, error) {
	splitLen := len(split)
	if splitLen < 2 {
		return nil, errors.New("length of split too short")
	}
	...
	// - sum of split amounts is enough to fulfill amount in invoice
	...
	if splitSum < invoiceAmount {
		return nil, fmt.Errorf("sum of split amounts '%v' is not enough to fulfill invoice amount of '%v'",
